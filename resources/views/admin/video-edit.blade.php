        ...
        <div class="card-body p-0 col-9">
          ...
          <form action="{{ route('video.update',['id'=>$video->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
              ...
              <div class="form-group">
                <label>Status</label>
                <select name="strike_id" id="strike_id" class="custom-select">
                    <option selected="selected" disabled>Выберите статус</option>
                  @foreach($strikes as $strike)
                    @if($strike->id == $video->strike->id)
                    <option value="{{ $strike->id }}" selected>{{ $strike->strike_name }}</option>
                    @else
                    <option value="{{ $strike->id }}">{{ $strike->strike_name }}</option>
                    @endif
                  @endforeach
                </select>
              </div>
